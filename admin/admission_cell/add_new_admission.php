...
$Clean['FeeGroupID'] = 4;  // 4 for new student 1 for old student
$Clean['FeeCode'] = '';
...
{
	case 1:	
	    if (isset($_POST['hdnStudentRegistrationID']))
        {
        ...
        if (isset($_POST['drdFeeGroup']))
        {
            $Clean['FeeGroupID'] = (int) $_POST['drdFeeGroup'];
        }
        ...
        if (isset($_POST['drdReligion']))
        {
            $Clean['Religion'] = strip_tags(trim($_POST['drdReligion']));
        }
		...
		{
		    $NewRecordValidator->ValidateStrings($Clean['EnrollmentID'], 'Sr No. is required and should be between 3 and 50 characters.', 3, 50);
		}
        
        if($Clean['FeeCode'] != '')
        {
            $NewRecordValidator->ValidateStrings($Clean['FeeCode'], 'Fee code should be between 3 and 20 characters.', 3, 20);   
        }
        ...
        if ($Clean['Religion'] != '') 
        {
            $NewRecordValidator->ValidateInSelect($Clean['Religion'], $ReligionList, 'Unknown error, please try again.');
        }
		...
		{
		    $StudentRegistrationObj = new StudentRegistration();
		}
		else
		{
		    $StudentRegistrationObj = new StudentRegistration($Clean['StudentRegistrationID']);
		}
    
        $StudentRegistrationObj->SetClassID($Clean['ClassID']);
        $StudentRegistrationObj->SetFirstName($Clean['FirstName']);
        $StudentRegistrationObj->SetLastName($Clean['LastName']);

        if ($Clean['DOB'] != '')
        {
            $StudentRegistrationObj->SetDOB(date('Y-m-d', strtotime(DateProcessing::ToggleDateDayAndMonth(($Clean['DOB'])))));
        }

        $StudentRegistrationObj->SetGender($Clean['Gender']);        
        $StudentRegistrationObj->SetBloodGroup($Clean['BloodGroup']);
        $StudentRegistrationObj->SetCategory($Clean['Category']);

        $StudentRegistrationObj->SetIsEWS($Clean['IsEWS']);
        $StudentRegistrationObj->SetHasDisability($Clean['HasDisability']);
        $StudentRegistrationObj->SetIsSingleGirl($Clean['IsSingleGirl']);

        $StudentRegistrationObj->SetFatherName($Clean['FatherFirstName']);
        $StudentRegistrationObj->SetMotherName($Clean['MotherFirstName']);

        $StudentRegistrationObj->SetResidentAddress($Clean['Address1']);
        $StudentRegistrationObj->SetPermanentAddress($Clean['Address2']);

        $StudentRegistrationObj->SetCountryID($Clean['CountryID']);
        $StudentRegistrationObj->SetStateID($Clean['StateID']);
        $StudentRegistrationObj->SetDistrictID($Clean['DistrictID']);
        $StudentRegistrationObj->SetCityID($Clean['CityID']);
        $StudentRegistrationObj->SetPinCode($Clean['PinCode']);

        $StudentRegistrationObj->SetAadharNumber($Clean['AadharNumber']);        
        $StudentRegistrationObj->SetMobileNumber($Clean['MobileNumber']);
        $StudentRegistrationObj->SetEmail($Clean['Email']);
        
        // Last Academic Details

        if($Clean['LastClass'] != '')
        {
            $StudentRegistrationObj->SetLastClass($Clean['LastClass']);       
        }

        $StudentRegistrationObj->SetLastSchool($Clean['LastSchool']);
        $StudentRegistrationObj->SetLastSchoolBoard($Clean['LastSchoolBoard']);
        $StudentRegistrationObj->SetTCReceived($Clean['TCReceived']);

        if ($Clean['TCDate'] != '')
        {
            $StudentRegistrationObj->SetTCDate(date('Y-m-d', strtotime(DateProcessing::ToggleDateDayAndMonth(($Clean['TCDate'])))));
        }
    
        $StudentRegistrationObj->SetSubjectsProposed($Clean['SubjectsProposed']);
        $StudentRegistrationObj->SetMotherTongue($Clean['MotherTongue']);
        $StudentRegistrationObj->SetHomeTown($Clean['HomeTown']);

        $StudentRegistrationObj->SetLastExamStatus($Clean['LastExamStatus']);
        $StudentRegistrationObj->SetLastExamPercentage($Clean['LastExamPercentage']);

        $StudentRegistrationObj->SetIsAdmissionTaken(1);
        $StudentRegistrationObj->SetRegistrationFee($Clean['RegistrationFee']);
        $StudentRegistrationObj->SetAcademicYearID($Clean['AcademicYearID']);
		
		$StudentRegistrationObj->SetCreateUserID($LoggedUser->GetUserID());
        $StudentRegistrationObj->SetIsActive(1);

        if ($Clean['AadharNumber'] != '') 
        {
            if ($StudentRegistrationObj->AadharExist())
            {
		...
		if (!$StudentRegistrationObj->Save())
		{
			$NewRecordValidator->AttachTextError(ProcessErrors($StudentRegistrationObj->GetLastErrorCode()));
			$HasErrors = true;
			break;
		}
        ...
        $NewStudentDetail->SetStudentRegistrationID($StudentRegistrationObj->GetStudentRegistrationID());
        $NewStudentDetail->SetAcademicYearID($Clean['AcademicYearID']);
        ...
        $NewStudentDetail->SetReligion($Clean['Religion']);
        ...
        $NewStudentDetail->SetStudentFeeGroupID($Clean['FeeGroupID']);
		...
		header('location:add_new_admission.php?Mode=ED');
		exit;
	break;
	...
	case 2:
        ...
        try
        {
            $RegisteredStudentDetails = new StudentRegistration($Clean['StudentRegistrationID']);
        }
        ...
        $Clean['ClassID'] = $RegisteredStudentDetails->GetClassID();
        
        $ClassSectionsList = ClassSections::GetClassSections($Clean['ClassID']);
        
        $Clean['FirstName'] = $RegisteredStudentDetails->GetFirstName();
        $Clean['LastName'] = $RegisteredStudentDetails->GetLastName();
        
        if ($RegisteredStudentDetails->GetDOB() != '0000-00-00')
        {
            $Clean['DOB'] = date('d/m/Y', strtotime($RegisteredStudentDetails->GetDOB()));   
        }

        $Clean['Gender'] = $RegisteredStudentDetails->GetGender();        
        $Clean['BloodGroup'] = $RegisteredStudentDetails->GetBloodGroup();
        $Clean['Category'] = $RegisteredStudentDetails->GetCategory();

        $Clean['IsEWS'] = $RegisteredStudentDetails->GetIsEWS();
        $Clean['HasDisability'] = $RegisteredStudentDetails->GetHasDisability();
        $Clean['IsSingleGirl'] = $RegisteredStudentDetails->GetIsSingleGirl();

        $Clean['FatherFirstName'] = $RegisteredStudentDetails->GetFatherName();
        $Clean['MotherFirstName'] = $RegisteredStudentDetails->GetMotherName();

        $Clean['Address1'] = $RegisteredStudentDetails->GetResidentAddress();
        $Clean['Address2'] = $RegisteredStudentDetails->GetPermanentAddress();
        
        $Clean['ResidentailAddress'] = $RegisteredStudentDetails->GetResidentAddress();
        $Clean['PermanentAddress'] = $RegisteredStudentDetails->GetPermanentAddress();

        $Clean['CountryID'] = $RegisteredStudentDetails->GetCountryID();
        $Clean['StateID'] = $RegisteredStudentDetails->GetStateID();
        $Clean['DistrictID'] = $RegisteredStudentDetails->GetDistrictID();
        $Clean['CityID'] = $RegisteredStudentDetails->GetCityID();
        
        $StateList = State::GetAllStates($Clean['CountryID']);
        $DistrictList = City::GetAllDistricts($Clean['StateID']);
        $CityList = City::GetAllCities($Clean['StateID'], $Clean['DistrictID']);
        
        $Clean['PinCode'] = $RegisteredStudentDetails->GetPinCode();
        
        $Clean['ResidentailCountryID'] = $RegisteredStudentDetails->GetCountryID();
        $Clean['ResidentailStateID'] = $RegisteredStudentDetails->GetStateID();
        $Clean['ResidentailDistrictID'] = $RegisteredStudentDetails->GetDistrictID();
        $Clean['ResidentailCityID'] = $RegisteredStudentDetails->GetCityID();
        
        $ResidentailStateList = State::GetAllStates($Clean['CountryID']);
        $ResidentailDistrictList = City::GetAllDistricts($Clean['StateID']);
        $ResidentailCityList = City::GetAllCities($Clean['StateID'], $Clean['DistrictID']);
        
        $Clean['ResidentailPinCode'] = $RegisteredStudentDetails->GetPinCode();
        
        $Clean['PermanentCountryID'] = $RegisteredStudentDetails->GetCountryID();
        $Clean['PermanentStateID'] = $RegisteredStudentDetails->GetStateID();
        $Clean['PermanentDistrictID'] = $RegisteredStudentDetails->GetDistrictID();
        $Clean['PermanentCityID'] = $RegisteredStudentDetails->GetCityID();
        
        $PermanentStateList = State::GetAllStates($Clean['CountryID']);
        $PermanentDistrictList = City::GetAllDistricts($Clean['StateID']);
        $PermanentCityList = City::GetAllCities($Clean['StateID'], $Clean['DistrictID']);
        
        $Clean['PermanentPinCode'] = $RegisteredStudentDetails->GetPinCode();
        
        $Clean['ParentAadharNumber'] = $RegisteredStudentDetails->GetAadharNumber();        
        $Clean['MobileNumber'] = $RegisteredStudentDetails->GetMobileNumber();
        $Clean['FatherMobileNumber'] = $RegisteredStudentDetails->GetMobileNumber();
        
        $Clean['Email'] = $RegisteredStudentDetails->GetEmail();
        $Clean['FatherEmail'] = $RegisteredStudentDetails->GetEmail();
        
        // Last Academic Details

        $Clean['LastClass'] = $RegisteredStudentDetails->GetLastClass();

        $Clean['LastSchool'] = $RegisteredStudentDetails->GetLastSchool();
        $Clean['LastSchoolBoard'] = $RegisteredStudentDetails->GetLastSchoolBoard();
        $Clean['TCReceived'] = $RegisteredStudentDetails->GetTCReceived();
        
        if ($RegisteredStudentDetails->GetTCDate() != '0000-00-00')
        {
            $Clean['TCDate'] = date('d/m/Y', strtotime($RegisteredStudentDetails->GetTCDate()));   
        }
    
        $Clean['SubjectsProposed'] = $RegisteredStudentDetails->GetSubjectsProposed();
        $Clean['MotherTongue'] = $RegisteredStudentDetails->GetMotherTongue();
        $Clean['HomeTown'] = $RegisteredStudentDetails->GetHomeTown();

        $Clean['LastExamStatus'] = $RegisteredStudentDetails->GetLastExamStatus();
        $Clean['LastExamPercentage'] = $RegisteredStudentDetails->GetLastExamPercentage();

        $Clean['RegistrationFee'] = $RegisteredStudentDetails->GetRegistrationFee();
        $Clean['AcademicYearID'] = $RegisteredStudentDetails->GetAcademicYearID();
        
        $Clean['IsActive'] = $RegisteredStudentDetails->GetIsActive();
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Add New Admission</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form class="form-horizontal" name="AddNewAdmission" action="add_new_admission.php" method="post" enctype="multipart/form-data">
...
?>
                    	...
                    	<div class="form-group">
                          ...
                          <div class="form-group">
                            ...
                            <div class="col-lg-4">
                            	<select class="form-control" name="drdFeeGroup" id="FeeGroup">
<?php
                                foreach ($ActiveFeeGroups as $FeeGroupID => $FeeGroup) 
                                {
...
?>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-4">
                                <select class="form-control" name="drdReligion" id="Religion">
                                    <option value="">Select Religion</option>
<?php
                                if (is_array($ReligionList) && count($ReligionList) > 0)
                                {
                                    foreach ($ReligionList as $ReligionId => $ReligionName) 
                                    {
                                        echo '<option ' . ($Clean['Religion'] == $ReligionId ? 'selected="selected"' : '') . ' value="' . $ReligionId . '">' . $ReligionName . '</option>';
                                    }
                                }
?>
                                </select>
...
<?php
                            ...
                            {
?>                              
                                <label class="col-sm-4"><input class="custom-radio" type="radio" id="<?php echo $LastExamStatusID; ?>" name="optLastExamStatus" value="<?php echo $LastExamStatusID; ?>" <?php echo ($Clean['LastExamStatus'] == $LastExamStatusID ? 'checked="checked"' : ''); ?> >&nbsp;&nbsp;<?php echo $LastExamStatusName; ?></label>            
<?php                                       
                            }
?>
                          </div>
...
// Calculate Age
    ...
    $("input[name='chkParentType']").click(function() {
        ...
        {
            var URL = '../school_administration/search_existing_parent.php';
