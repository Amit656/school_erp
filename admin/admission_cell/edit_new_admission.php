...
switch ($Clean['Process'])
{
    case 3: 
        ...
        $StudentRegistrationObj = new StudentRegistration($StudentToEdit->GetStudentRegistrationID());
        ...
        $StudentToEdit->SetStudentRegistrationID($StudentRegistrationObj->GetStudentRegistrationID());
        $StudentToEdit->SetAcademicYearID($Clean['AcademicYearID']);
        ...
        $StudentToEdit->SetReligion($Clean['Religion']);
        ...
        // move_uploaded_file($Clean['UploadFile']['tmp_name'], $UniqueUserFileUploadDirectory . $StudentToEdit->GetStudentID().'.jpg');

            move_uploaded_file($Clean['UploadFile']['tmp_name'], $UniqueUserFileUploadDirectory . $FileName);
        }   
        
        header('location:new_admission_list.php?Mode=ED');
        exit;
    break;
    ...
    case 2:
        ...
        if ($StudentToEdit->GetParentID() > 0)
        {
            $Clean['ParentType'] = 'Existing';
        }

        $NewParentDetail = new ParentDetail($StudentToEdit->GetParentID());

        $Clean['ParentID'] = $NewParentDetail->GetParentID();
...
?>
<title>Edit New Admission</title>
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Edit New Admission</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form class="form-horizontal" name="EditStudent" action="edit_new_admission.php" method="post" enctype="multipart/form-data">
...
<?php
                                ...
                                {
                                    echo '<option value="' . $FeeGroupID . '">' . $FeeGroup . '</option>';
                                }
?>
                                </select>
                            </div>
