...
$AdmissionConfirmList = array(0 => 'Pending', 1 => 'Confirmed');
...
switch ($Clean['Process'])
{
	case 5:
		...
		// {
		// 	$RecordValidator->AttachTextError('This student registration cannot be deleted. There are dependent records for this student registration.');
		// 	$HasErrors = true;
		// 	break;
		// }
		...
		header('location:student_registrations_list.php?Mode=DD');
	    break;

    case 7:
        if (isset($_GET['drdAcademicYear']))
        {
            $Clean['AcademicYearID'] = $_GET['drdAcademicYear'];
        }
        ...
        if (isset($_GET['optIsAdmissionConfirm']))
        {
            $Clean['IsAdmissionConfirm'] = (int) $_GET['optIsAdmissionConfirm'];
        }
        else if (isset($_GET['IsAdmissionConfirm']))
        {
            $Clean['IsAdmissionConfirm'] = (int) $_GET['IsAdmissionConfirm'];
        }
        ...
        $RecordValidator->ValidateInSelect($Clean['IsAdmissionConfirm'], $AdmissionConfirmList, 'Unknown error, please try again.');
        ...
        $Filters['IsAdmissionConfirm'] = $Clean['IsAdmissionConfirm'];

        //get records count
        StudentRegistration::SearchStudentRegistration($TotalRecords, true, $Filters);
        ...
        if ($TotalRecords > 0)
        {
            ...
            {
                $RegisteredStudents = StudentRegistration::SearchStudentRegistration($TotalRecords, false, $Filters, $Start, $TotalRecords);
            }
            else
            {
                $RegisteredStudents = StudentRegistration::SearchStudentRegistration($TotalRecords, false, $Filters, $Start, $Limit);
            }
        }
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Student Registration List</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <strong>Filter Student Registration List</strong>
                        </div>
<?php
                        if ($HasErrors == true)
                        {
...
?>
                         <div class="panel-body">
                            <div>
                                <div class="row" >
                                    <div class="col-lg-12">
                                         <form class="form-horizontal" name="FilterStudentRegistration" id="FilterStudentRegistration" action="student_registrations_list.php" method="get">
...
?>
                                                ...
                                                <label for="IsAdmissionConfirm" class="col-lg-2 control-label">Admission</label>
                                                <div class="col-lg-4">
<?php
                                                foreach ($AdmissionConfirmList as $Status => $StatusName) 
                                                {
?>
                                                    <label style="font-weight: normal;">
                                                        <input type="radio" id="IsAdmissionConfirm" name="optIsAdmissionConfirm" value="<?php echo $Status; ?>" <?php echo($Clean['IsAdmissionConfirm'] == $Status ? 'checked="checked"' : '');?> />&nbsp;<?php echo $StatusName;?>&nbsp;
                                                    </label>
<?php
                                                }
?>
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div>
                                <div class="row">
                                    <div class="col-lg-3">
                                        <div class="add-new-btn-container"><a href="add_student_registration.php" class="btn btn-primary<?php echo $LoggedUser->HasPermissionForTask(TASK_ADD_STUDENT_REGISTRATION) === true ? '' : ' disabled'; ?>" role="button">Add New Student Registration</a></div>
                                    </div>
                                    <div class="col-lg-6">
<?php
                                    if ($TotalPages > 1)
                                    {
                                        $AllParameters = array('Process' => '7', 'AcademicYearID' => $Clean['AcademicYearID'], 'ClassID' => $Clean['ClassID'], 'StudentName' => $Clean['StudentName'], 'Genders' => $Clean['Gender'], 'MobileNumber' => $Clean['MobileNumber'], 'FatherName' => $Clean['FatherName'], 'MotherName' => $Clean['MotherName'], 'Categories' => $Clean['Category'], 'IsAdmissionConfirm' => $Clean['IsAdmissionConfirm'], 'IsActive' => $Clean['IsActive']);
                                        echo UIHelpers::GetPager('student_registrations_list.php', $TotalPages, $Clean['CurrentPage'], $AllParameters);
                                    }
?>
                                    </div>
                                    <div class="col-lg-3">
...
?>
                                <div class="row" id="RecordTableHeading">
                                    <div class="col-lg-12">
                                        <div class="report-heading-container"><strong>Student Registrations on <?php echo date('d-m-Y h:i A'); ?></strong></div>
                                    </div>
                                </div>
                                <div class="row" id="RecordTable">
                                    <div class="col-lg-12">
                                        <table width="100%" class="table table-striped table-bordered table-hover" id="DataTableRecords">
                                            ...
                                            <tbody>
<?php
                                    if (is_array($RegisteredStudents) && count($RegisteredStudents) > 0)
                                    {
                                        $Counter = 0;
...
?>
                                                <tr>
                                                    ...
                                                    <td><?php echo $StudentRegistrationDetails['ClassName']; ?></td>
                                                    <td><?php echo $StudentRegistrationDetails['StudentName']; ?></td>
                                                    <td><?php echo $StudentRegistrationDetails['Gender']; ?></td>
                                                    <td><?php echo $StudentRegistrationDetails['Category']; ?></td>
                                                    <td><?php echo (($StudentRegistrationDetails['IsAdmissionTaken']) ? 'Yes' : 'No'); ?></td>
                                                    <td><?php echo (($StudentRegistrationDetails['RegistrationFee']) ? $StudentRegistrationDetails['RegistrationFee'] : ''); ?></td>
                                                    <td><?php echo ($StudentRegistrationDetails['AadharNumber']) ? $StudentRegistrationDetails['AadharNumber'] : '-'; ?></td>
                                                    <td><?php echo $StudentRegistrationDetails['FatherName']; ?></td>
                                                    <td><?php echo $StudentRegistrationDetails['MotherName']; ?></td>
                                                    <td><?php echo $StudentRegistrationDetails['MobileNumber']; ?></td>
                                                    <td><?php echo (($StudentRegistrationDetails['IsActive']) ? 'Yes' : 'No'); ?></td>
                                                    <td><?php echo $StudentRegistrationDetails['CreateUserName']; ?></td>
                                                    <td><?php echo date('d/m/Y', strtotime($StudentRegistrationDetails['CreateDate'])); ?></td>
                                                    <td class="print-hidden">
<?php
                                                    if ($LoggedUser->HasPermissionForTask(TASK_EDIT_STUDENT_REGISTRATION) === true)
                                                    {
                                                        echo '<a href="edit_student_registration.php?Process=2&amp;StudentRegistrationID='. $StudentRegistrationID .'">Edit</a>';
                                                    }
                                                    else
                                                    {
                                                        echo 'Edit';
                                                    ...
                                                    if ($LoggedUser->HasPermissionForTask(TASK_DELETE_STUDENT_REGISTRATION) === true)
                                                    {
                                                        echo '<a class="delete-record" href="student_registrations_list.php?Process=5&amp;StudentRegistrationID=' . $StudentRegistrationID . '">Delete</a>';
                                                    }
                                                    else
                                                    {
                                                        echo 'Delete';
                                                    ...
                                                    if ($LoggedUser->HasPermissionForTask(TASK_DELETE_STUDENT_REGISTRATION) === true)
                                                    {
                                                        echo '<a href="add_new_admission.php?Process=2&amp;StudentRegistrationID=' . $StudentRegistrationID . '" target="_blank">Confirm</a>';
                                                    }
                                                    else
                                                    {
                                                        echo 'Confirm';
