    ...
    case 1:
        ...
        for ($row = $Clean['StartFromRow']; $row <= $highestRow - 1; $row++)
        {
            ...
            foreach ($rowData as $key => $Details)
            {
				...
				if (isset($Supplier))
				{
					$RowData['Supplier'] = strip_tags(trim($Supplier));
				}
				if (isset($Details[3]))
				{
				...
				if (!$RowValidator->ValidateStrings($RowData['Supplier'], '', 2, 250))
				{
					$ErrorRows[$row][] = 'Invalid supplier name. (Allowed 2 to 250 chars)';
				}
				
				if ($RowData['SupplierAddress'] != '' && !$RowValidator->ValidateStrings($RowData['SupplierAddress'], '', 2, 150))
				{
					$ErrorRows[$row][] = 'Invalid supplier address. (Allowed 2 to 150 chars)';
				}
				
				if ($RowData['VoucherNumber'] != '' && !$RowValidator->ValidateStrings($RowData['VoucherNumber'], '', 1, 50))
				{
					$ErrorRows[$row][] = 'Invalid voucher number. (Allowed 2 to 50 chars)';
				}
				
				if ($RowData['SupplierInvoiceNumber'] != '' && !$RowValidator->ValidateStrings($RowData['SupplierInvoiceNumber'], '', 2, 50))
				{
					$ErrorRows[$row][] = 'Invalid supplier invoice number. (Allowed 2 to 50 chars)';
				}
		...
		if (!UploadHelpers::SaveStockPurchaseFromExcel($AllRows))
		{
			$NewRecordValidator->AttachTextError('Error in uploaded file, please try again.');
			$HasErrors = true;
			break;
...
<body>
    ...
    <div id="wrapper">
        <div class="col-lg-12">
            <div class="row">
                <div class="col-lg-12">
                    <h3 class="page-header">Upload Inventory Data(Purchase)</h3>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form class="form-horizontal" name="AddStudent" action="upload_purchase_data.php" method="post" enctype="multipart/form-data">
...
<?php
		if (count($ErrorRows) > 0) 
        {
			echo '<div class="alert alert-success" role="alert">Total records found to be uploaded: <strong>'. count($AllRows) .'</strong></div>';
