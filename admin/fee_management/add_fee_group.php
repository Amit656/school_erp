...
switch ($Clean['Process'])
{
	case 1:
		...
		$NewFeeGroup = new FeeGroup();
						
		$NewFeeGroup->SetFeeGroup($Clean['FeeGroup']);

		$NewFeeGroup->SetIsActive(1);
		$NewFeeGroup->SetCreateUserID($LoggedUser->GetUserID());

		if (!$NewFeeGroup->Save())
		{
			$NewRecordValidator->AttachTextError(ProcessErrors($NewFeeGroup->GetLastErrorCode()));
			$HasErrors = true;
			break;
		}
		
		header('location:fee_group_list.php?Mode=AS');
		exit;
	break;
}
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Add Fee Group</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form class="form-horizontal" name="AddFeeGroup" action="add_fee_group.php" method="post">
