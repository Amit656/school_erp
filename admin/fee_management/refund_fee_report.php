...
switch ($Clean['Process'])
{
    case 7:         
        ...
        foreach ($Clean['MonthList'] as $Key => $MonthID)
        {
            ...
            $TotalMonthlyFeeAmount = $StudentDetailObject->GetStudentMonthlyFeeDetails($MonthID, $StudentMonthlyFeeDetails, true);
        ...
        //get records count
        FeeTransaction::SearchRefundFeeReport($TotalRecords, true, $Filters);
        ...
        if ($TotalRecords > 0)
        {
            ...
            {
                $RefundFeeReportDetails = FeeTransaction::SearchRefundFeeReport($TotalRecords, false, $Filters, 0, $TotalRecords);
            }
            else
            {
                $RefundFeeReportDetails = FeeTransaction::SearchRefundFeeReport($TotalRecords, false, $Filters, $Start, $Limit);
            }
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Refund Student Fee Report</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <form class="form-horizontal" name="frmRoomReport" action="refund_fee_report.php" method="get">
...
?>
            ...
            <!-- /.row -->
<?php
        if (($Clean['Process'] == 7 || $Clean['Process'] == 1))
        {
            $ReportHeaderText = '';
...
<?php
                                    if ($TotalPages > 1)
                                    {
                                        ...
                                        echo UIHelpers::GetPager('refund_fee_report.php', $TotalPages, $Clean['CurrentPage'], $AllParameters);
                                    }
?>                                        
                                    </div>
                                </div>
                                ...
                                <div class="row" id="RecordTable">
                                    <div class="col-lg-12">
                                        <table width="100%" class="table table-striped table-bordered table-hover" id="DataTableRecords">
                                            ...
                                            <tbody>
<?php
                                    if (is_array($RefundFeeReportDetails) && count($RefundFeeReportDetails) > 0)
                                    {
                                        $Counter = $Start;

                                        foreach ($RefundFeeReportDetails as $RefundFeeID => $Details)
                                        {
?>
                                            <tr>
                                                ...
                                                <td><?php echo number_format($Details['TotalRefundAmount'], 2); ?></td>
                                                <td><?php echo number_format($Details['SubmittedHeadAmount'], 2); ?></td>
                                            </tr>
<?php
                                        }                                          
                                    }
