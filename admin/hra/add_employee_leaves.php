...
if ($LoggedUser->HasPermissionForTask(TASK_ADD_EMPLOYEE_LEAVE) !== true)
{
...
switch ($Clean['Process'])
{
	case 1:
		...
		$NewEmployeeLeave->SetBranchStaffID($Clean['BranchStaffID']);
		$NewEmployeeLeave->SetEmployeeLeaveDetails($EmployeeLeaveDetails);
		
		$NewEmployeeLeave->SetCreateUserID($LoggedUser->GetUserID());

		if (!$NewEmployeeLeave->Save())
		{
			$NewRecordValidator->AttachTextError(ProcessErrors($NewEmployeeLeave->GetLastErrorCode()));
			$HasErrors = true;

			break;
		}

		header('location:employee_leaves_list.php?Mode=ED&Process=7&StaffCategory=' . $Clean['StaffCategory']);
		exit;
		break;
	...
	case 11:
		...
		if (strtotime(date('Y-m-d',strtotime(DateProcessing::ToggleDateDayAndMonth($Clean['LeaveDateFrom'])))) < strtotime(date('Y-m-d')))
        {
        	$NewRecordValidator->AttachTextError('From date must be greater than todays date.');	
        }
...
?>                    
					<form class="form-horizontal" name="SearchEmployeeLeave" action="add_employee_leaves.php" method="post">
...
<?php
		if($Clean['Process'] == 1 || $Clean['Process'] == 7 || $Clean['Process'] == 11)
		{
?>
			<div class="panel panel-default">
                <div class="panel-heading">
                ...
                <div class="panel-body">
                	<form class="form-horizontal" name="AddEmployeeLeaves" action="add_employee_leaves.php" method="post">
						<div class="form-group">
                            <label for="BranchStaffName" class="col-lg-2 control-label">Name</label>
                            <div class="col-lg-4">
...
?>
    		...
    		<div class="panel panel-default">
                	...
                	<div class="panel-body">
                		<form  class="form-horizontal" name="EmployeeLeaveRequest" action="add_employee_leaves.php" method="post">
	                        <div class="form-group">
...
<?php
					if($Clean['Process'] == 11 || $Clean['Process'] == 1)
					{
?>
						<form  name="AddLeave" action="add_employee_leaves.php" method="post">
	                        <div class="row" id="RecordTable">
	                            <div class="col-lg-12">
