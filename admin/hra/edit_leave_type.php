...
elseif (isset($_POST['hdnLeaveTypeID']))
{
    $Clean['LeaveTypeID'] = (int) $_POST['hdnLeaveTypeID'];
}
...
try
{
    $LeaveTypeToEdit = new LeaveType($Clean['LeaveTypeID']);
}
...
switch ($Clean['Process'])
{
	case 3:
		...
		$LeaveTypeToEdit->SetStaffCategory($Clean['StaffCategory']);
		$LeaveTypeToEdit->SetLeaveType($Clean['LeaveType']);
		$LeaveTypeToEdit->SetNoOfLeaves($Clean['NoOfLeaves']);

		$LeaveTypeToEdit->SetLeaveMode($Clean['LeaveMode']);
		$LeaveTypeToEdit->SetLeavePayType($Clean['LeavePayType']);
		$LeaveTypeToEdit->SetMonthlyCarryForward($Clean['MonthlyCarryForward']);

		$LeaveTypeToEdit->SetPriority($Clean['Priority']);

		$LeaveTypeToEdit->SetCreateUserID($LoggedUser->GetUserID());

		if ($LeaveTypeToEdit->RecordExists())
		{
		...
		if (!$LeaveTypeToEdit->Save())
		{
			$NewRecordValidator->AttachTextError(ProcessErrors($LeaveTypeToEdit->GetLastErrorCode()));
			$HasErrors = true;
			break;
		}
	...
	case 2:
	    $Clean['StaffCategory'] = $LeaveTypeToEdit->GetStaffCategory();
	    $Clean['LeaveType'] = $LeaveTypeToEdit->GetLeaveType();
	    $Clean['NoOfLeaves'] = $LeaveTypeToEdit->GetNoOfLeaves();

	    $Clean['LeaveMode'] = $LeaveTypeToEdit->GetLeaveMode();
	    $Clean['LeavePayType'] = $LeaveTypeToEdit->GetLeavePayType();
	    $Clean['MonthlyCarryForward'] = $LeaveTypeToEdit->GetMonthlyCarryForward();
	    
	    $Clean['Priority'] = $LeaveTypeToEdit->GetPriority();
    	break;
}
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
             ...
             <form class="form-horizontal" name="EditLeaveTypes" action="edit_leave_type.php" method="post">
...
?>
                        ...
                        <div class="col-sm-offset-2 col-lg-10">
                        	...
                        	<input type="hidden" name="hdnLeaveTypeID" value="<?php echo $Clean['LeaveTypeID']; ?>" />
