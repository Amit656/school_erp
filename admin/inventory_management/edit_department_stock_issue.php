...
elseif (isset($_POST['hdnDepartmentStockIssueID']))
{
    $Clean['DepartmentStockIssueID'] = (int) $_POST['hdnDepartmentStockIssueID'];
}
...
try
{
    $DepartmentStockIssueToEdit = new DepartmentStockIssue($Clean['DepartmentStockIssueID']);
}
...
$ProductStockQuantity = GetStockQunatity($Clean['ProductID']);
...
switch ($Clean['Process'])
{
	case 3:
		...
		$DepartmentStockIssueToEdit->SetDepartmentID($Clean['DepartmentID']);
		$DepartmentStockIssueToEdit->SetProductID($Clean['ProductID']);
		$DepartmentStockIssueToEdit->SetIssuedToType($Clean['IssuedToType']);
		$DepartmentStockIssueToEdit->SetIssuedToID($Clean['IssuedToID']);
		$DepartmentStockIssueToEdit->SetIssuedQuantity($Clean['IssuedQuantity']);
		$DepartmentStockIssueToEdit->SetIssueDate(date('Y-m-d',strtotime(DateProcessing::ToggleDateDayAndMonth($Clean['IssueDate']))));
		$DepartmentStockIssueToEdit->SetReturnDate($ReturnDate);
		
		$DepartmentStockIssueToEdit->SetCreateUserID($LoggedUser->GetUserID());

		// if ($DepartmentStockIssueToEdit->RecordExists())
  //       {
  ...
  //       }
		...
		if (!$DepartmentStockIssueToEdit->Save())
		{
			$NewRecordValidator->AttachTextError(ProcessErrors($DepartmentStockIssueToEdit->GetLastErrorCode()));
			$HasErrors = true;

			break;
		}

		header('location:department_stock_issues_list.php?Mode=UD');
		exit;
		break;
	...
	case 2:
		...
		$Clean['DepartmentID'] = $DepartmentStockIssueToEdit->GetDepartmentID();
		$Clean['ProductID'] = $DepartmentStockIssueToEdit->GetProductID();

		$ProductStockQuantity = StockIssue::GetStockQunatity($Clean['ProductID']);

		$Clean['IssuedToType'] = $DepartmentStockIssueToEdit->GetIssuedToType();

		if ($Clean['IssuedToType'] == 'Student') 
		{
			$Clean['IssuedToID'] = $DepartmentStockIssueToEdit->GetIssuedToID();

			$StudentDetails = new StudentDetail($Clean['IssuedToID']);
			$Clean['ClassSectionID'] = $StudentDetails->GetClassSectionID();			

			if (AddedClass::GetClassIDSectionIDByClassSectionID($Clean['ClassSectionID'], $ClassID, $SectionMasterID))
			{
				$Clean['ClassID'] = $ClassID;
			}
		...
		else
		{
			...
			$BranchStaffDetails = new BranchStaff($Clean['IssuedToID']);
			$Clean['StaffCategory'] = $BranchStaffDetails->GetStaffCategory();

			$AllBranchStaffList = BranchStaff::GetActiveBranchStaff($Clean['StaffCategory']);
		}
		
		$Clean['IssuedQuantity'] = $DepartmentStockIssueToEdit->GetIssuedQuantity();
		$Clean['IssueDate'] = $DepartmentStockIssueToEdit->GetIssueDate();
		$Clean['ReturnDate'] = $DepartmentStockIssueToEdit->GetReturnDate();
		break;
}

require_once('../html_header.php');
?>
<title>Edit Department Stock Issue</title>
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Edit Department Stock Issue</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form class="form-horizontal" name="EditDepartmentStockIssue" action="edit_department_stock_issue.php" method="post">
...
?>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-4">
                            	<input class="form-control dtepicker" type="text" name="txtIssueDate" value="<?php echo ($Clean['IssueDate']) ? date('d/m/Y', strtotime($Clean['IssueDate'])) : ''; ?>"/>	
                            </div>
	                        <label for="ReturnDate" class="col-lg-2 control-label">Return Date</label>
                            <div class="col-lg-4">
                            	<input class="form-control dtepicker" type="text" name="txtReturnDate" value="<?php echo ($Clean['ReturnDate']) ? date('d/m/Y', strtotime($Clean['ReturnDate'])) : ''; ?>"/>	
                            </div>
	                    </div>
                        <div class="form-group">
                        <div class="col-sm-offset-2 col-lg-10">
                        	<input type="hidden" name="hdnProcess" value="3" />
                        	<input type="hidden" name="hdnDepartmentStockIssueID" value="<?php echo $Clean['DepartmentStockIssueID'];?>" />
