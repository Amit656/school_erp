...
if ($LoggedUser->HasPermissionForTask(TASK_LIST_STOCK_ISSUE) !== true)
{
...
//$AllStockIssues = StockIssue::GetAllStockIssues();
...
{
	case 5:
		if ($LoggedUser->HasPermissionForTask(TASK_DELETE_STOCK_ISSUE) !== true)
		{
			header('location:unauthorized_login_admin.php');
			exit;
		}
		...
		try
		{
			$StockIssueToDelete = new StockIssue($Clean['StockIssueID']);
		}
		...
		// if ($StockIssueToDelete->CheckDependencies())
		// {
		...
		if (!$StockIssueToDelete->Remove())
		{
			$RecordValidator->AttachTextError(ProcessErrors($StockIssueToDelete->GetLastErrorCode()));
			$HasErrors = true;
			break;
		}

		header('location:stock_issues_list.php?Mode=DD');
		break;
	...
	case 7;
		...
		StockIssue::GetAllStockIssue($TotalRecords, true, $Filters);
		...
		if ($TotalRecords > 0)
		{
			...
			{
				$AllStockIssues = StockIssue::GetAllStockIssue($TotalRecords, false, $Filters, 0, $TotalRecords);
			}
			else
			{
				$AllStockIssues = StockIssue::GetAllStockIssue($TotalRecords, false, $Filters, $Start, $Limit);
			}
		}
		break;
}
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Stock Issue List</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
			<form class="form-horizontal" name="SearchStockIssue" action="stock_issues_list.php" method="get">
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div>
                                <div class="row">
                                    <div class="col-lg-6">
										<div class="add-new-btn-container"><a href="add_stock_issue.php" class="btn btn-primary<?php echo $LoggedUser->HasPermissionForTask(TASK_ADD_STOCK_ISSUE) === true ? '' : ' disabled'; ?>" role="button">Add New Stock Issue</a></div>
                                    </div>
                                    <div class="col-lg-6">
								...
								if ($TotalPages > 1)
								{
									...
									echo UIHelpers::GetPager('stock_issues_list.php', $TotalPages, $Clean['CurrentPage'], $AllParameters);
								}
								?> 

                                <div class="row" id="RecordTableHeading">
                                    <div class="col-lg-12">
										<div class="report-heading-container"><strong>Stock Issues on <?php echo date('d-m-Y h:i A'); ?></strong></div>
                                    </div>
								</div>
												...
												if (is_array($AllStockIssues) && count($AllStockIssues) > 0)
												{
													...
													foreach ($AllStockIssues as $StockIssueID => $StockIssueDetails)
													{
														...
														<tr>
															...
															<td><?php echo $StockIssueDetails['StaffCategory']; ?></td>
															<td><?php echo (isset($StockIssueDetails['DepartmentName']) ? $StockIssueDetails['DepartmentName'] : $StockIssueDetails['BranchStaffName']) ?></td>
															<td><?php echo (isset($StockIssueDetails['VoucherNumber']) ? $StockIssueDetails['VoucherNumber'] : 'N/A'); ?></td>
															<td><?php echo (isset($StockIssueDetails['Description']) ? $StockIssueDetails['Description'] : 'N/A'); ?></td>
															<td><?php echo $StockIssueDetails['CreateUserName']; ?></td>
															<td><?php echo date('d/m/Y', strtotime($StockIssueDetails['CreateDate'])); ?></td>
															<td class="print-hidden">
																<?php
																if ($LoggedUser->HasPermissionForTask(TASK_EDIT_STOCK_ISSUE) === true)
																{
																	echo '<a href="edit_stock_issue.php?Process=2&amp;StockIssueID=' . $StockIssueID . '">Edit</a>';
																}
																else
																{
																	echo 'Edit';
																...
																if ($LoggedUser->HasPermissionForTask(TASK_EDIT_STOCK_ISSUE) === true)
																{
																	echo '<a href="edit_stock_issue.php?Process=2&amp;ViewOnly=true&amp;StockIssueID=' . $StockIssueID . '">View</a>';
																}
																else
																{
																	echo 'View';
																...
																{
																	echo '<a href="add_stock_return.php?Process=2&amp;StockIssueID=' . $StockIssueID . '">Return</a>';
																}
																else
																{
																	echo 'Return';
																...
																if ($LoggedUser->HasPermissionForTask(TASK_DELETE_STOCK_ISSUE) === true)
																{
																	echo '<a class="delete-record" href="stock_issues_list.php?Process=5&amp;StockIssueID=' . $StockIssueID . '">Delete</a>';
																}
																else
																{
																	echo 'Delete';
