...
else if (isset($_POST['hdnBookID']))
{
    $Clean['BookID'] = (int) $_POST['hdnBookID'];
}
...
try
{
    $BookToEdit = new Book($Clean['BookID']);
}
...
$AllreadyPresentAuthors = array();
$RemovedClassList = array();
$RemovedAuthorList = array();
...
switch ($Clean['Process'])
{
    case 3:     
        ...
        $AllReadyExistAuthorDetails = $BookToEdit->GetAuthorDetails();

        foreach ($AllReadyExistAuthorDetails as $BookAuthorID => $Details) 
        {
            if (!in_array($Details['AuthorID'], $Clean['AuthorIDList'])) 
            {
                $RemovedAuthorList[$BookAuthorID] = $Details['AuthorID'];
            }

            $AllreadyPresentAuthors[] = $Details['AuthorID'];           
        }       
        ...
        foreach ($Clean['AuthorNameList'] as $Counter => $AuthorName) 
        {
            ...
            if (in_array($AuthorName, array_column($AuthorDetails, 'AuthorName')) || in_array($Clean['AuthorIDList'][$Counter], array_column($AuthorDetails, 'AuthorID'))) 
            {
                $AllReadyExistAuthor[$Counter] = 'The author you enter, is allready in list.';
            }

            if (in_array($Clean['AuthorIDList'][$Counter], $AllreadyPresentAuthors)) 
            {
                unset($Clean['AuthorIDList'][$Counter]);
                continue;
            }
            ...
            else
            {
                ...
                $AuthorDetails[$Counter]['AuthorName'] = '';
            }
        }
        ...
        $AllReadyAssignedClasses = $BookToEdit->GetClassList();

        foreach ($AllReadyAssignedClasses as $BooksClassID => $ClassID) 
        {
            if (!in_array($ClassID, $Clean['ClassIDList'])) 
            {
                $RemovedClassList[$BooksClassID] = $ClassID;
            }
        }
        ...
        {                       
            ...
            {              
                foreach ($Clean['ClassIDList'] as $key => $ClassID) 
                {
                    ...
                    if (array_key_exists($ClassID, $AllReadyAssignedClasses)) 
                    {
                        unset($Clean['ClassIDList'][array_search($ClassID, $Clean['ClassIDList'])]);
                    }
                }               
            }
            else
        ...
        $BookToEdit->SetBookCategoryID($Clean['BookCategoryID']);

        $BookToEdit->SetISBN($Clean['ISBN']);
        $BookToEdit->SetBookName($Clean['BookName']);
        $BookToEdit->SetBookType($Clean['BookType']);
        $BookToEdit->SetIssuableTo($Clean['IssuableTo']);
        $BookToEdit->SetTakeHomeAllowed($Clean['TakeHomeAllowed']);        
        $BookToEdit->SetVolume($Clean['Volume']);
        $BookToEdit->SetNumberOfPages($Clean['NumberOfPages']); 

        $BookToEdit->SetPrice($Clean['Price']);
        $BookToEdit->SetPurchaseDate(date('Y-m-d', strtotime(DateProcessing::ToggleDateDayAndMonth(($Clean['PurchaseDate'])))));

        $PreviousQuantity = $BookToEdit->GetQuantity();

        if ($PreviousQuantity != $Clean['Quantity']) 
        {
            $BookToEdit->SetPreviousQuantity($PreviousQuantity);
        }
        
        $BookToEdit->SetQuantity($Clean['Quantity']);
        $BookToEdit->SetShelfNumber($Clean['ShelfNumber']);

        $BookToEdit->SetPublishedYear($Clean['PublishedYear']);
        $BookToEdit->SetEdition($Clean['Edition']);

        $BookToEdit->SetIsActive(1);
        $BookToEdit->SetCreateUserID($LoggedUser->GetUserID());

        $BookToEdit->SetClassList($Clean['ClassIDList']);
        $BookToEdit->SetAuthorDetails($AuthorDetails);        

        if ($Clean['PublisherID'] <= 0) 
        {
            $BookToEdit->SetPublisherName($Clean['PublisherName']);
        }
        else
        {
            $BookToEdit->SetPublisherID($Clean['PublisherID']);
        }

        $BookToEdit->SetRemovedClassList($RemovedClassList);
        $BookToEdit->SetRemovedAuthorList($RemovedAuthorList);
        
        if (!$BookToEdit->Save())
        {
            $NewRecordValidator->AttachTextError(ProcessErrors($BookToEdit->GetLastErrorCode()));
            $HasErrors = true;
            break;
        }
        
        header('location:book_report.php?Mode=UD');
        exit;
    break;

    case 2:            
        
        $Clean['BookCategoryID'] = $BookToEdit->GetBookCategoryID();

        try
        {
            $BookCategoryObject = new BookCategory($Clean['BookCategoryID']);
        }
        catch (ApplicationDBException $e)
        {
            header('location:/admin/error.php');
        ...
        $Clean['ParentCategoryID'] = $BookCategoryObject->GetParentCategoryID();

        $BookCategoryList = BookCategory::GetActiveSubCategories($Clean['ParentCategoryID']);

        $Clean['ISBN'] = $BookToEdit->GetISBN();
        $Clean['BookName'] = $BookToEdit->GetBookName();
        $Clean['BookType'] = $BookToEdit->GetBookType();
        $Clean['IssuableTo'] = $BookToEdit->GetIssuableTo();
        $Clean['TakeHomeAllowed'] = $BookToEdit->GetTakeHomeAllowed();        
        $Clean['Volume'] = $BookToEdit->GetVolume();
        $Clean['NumberOfPages'] = $BookToEdit->GetNumberOfPages(); 

        $Clean['Price'] = $BookToEdit->GetPrice();
        $Clean['PurchaseDate'] = date('d/m/Y', strtotime($BookToEdit->GetPurchaseDate()));
        $Clean['Quantity'] = $BookToEdit->GetQuantity();
        $Clean['ShelfNumber'] = $BookToEdit->GetShelfNumber();

        $Clean['PublishedYear'] = $BookToEdit->GetPublishedYear();
        $Clean['Edition'] = $BookToEdit->GetEdition();

        $Clean['IsActive'] = $BookToEdit->GetIsActive();
    
        $Clean['ClassIDList'] = $BookToEdit->GetClassList();

        $AuthorDetails = $BookToEdit->GetAuthorDetails();

        $Counter = 1;
        foreach ($AuthorDetails as $BookAuthorID => $Details) 
        {
            $Clean['AuthorIDList'][$Counter] = $Details['AuthorID'];
            $Clean['AuthorNameList'][$Counter] = $Details['AuthorName'];
            $Counter++;
        }

        $Clean['PublisherID'] = $BookToEdit->GetPublisherID();
        $Clean['PublisherName'] = $BookToEdit->GetPublisherName();
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
             ...
             <form class="form-horizontal" name="EditBookCategory" action="edit_book.php" method="post">
...
<?php
                                ...
                                {
                                    foreach($IssuableToList as $IssuableTo => $IssuableToName)
                                    {
                                        echo '<option ' . (($Clean['IssuableTo'] == $IssuableTo) ? 'selected="selected"' : '' ) . ' value="' . $IssuableTo . '">' . $IssuableToName . '</option>';
                                    }
                                }
?>
                                </select>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            <div class="col-sm-offset-2 col-lg-10">
                                ...
                                <input type="hidden" name="hdnBookID" value="<?php echo $Clean['BookID']; ?>" />
