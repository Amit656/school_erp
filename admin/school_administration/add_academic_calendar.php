...
if ($LoggedUser->HasPermissionForTask(TASK_ADD_ACADEMIC_CALENDER_EVENT) !== true)
{
...
switch ($Clean['Process'])
{
	case 1:		
        ...
        $AddAcademicCalendar = new AcademicCalendar();
        
        $AddAcademicCalendar->SetEventStartDate(DateProcessing::ToggleDateDayAndMonth($Clean['EventStartDate']));
        $AddAcademicCalendar->SetEventEndDate(DateProcessing::ToggleDateDayAndMonth($Clean['EventEndDate']));
     
        $AddAcademicCalendar->SetEventName($Clean['EventName']);
        $AddAcademicCalendar->SetEventDetails($Clean['EventDetails']);

        $AddAcademicCalendar->SetIsHoliday($Clean['IsHoliday']);
        $AddAcademicCalendar->SetToNotify($Clean['ToNotify']);

        $AddAcademicCalendar->SetNotificationMessage($Clean['NotificationMessage']);

        if ($Clean['ToNotify'] == 1 && $Clean['NotificationDate'] != '')
        {
           $AddAcademicCalendar->SetNotificationDate(DateProcessing::ToggleDateDayAndMonth($Clean['NotificationDate'])); 
        }   

        $AddAcademicCalendar->SetHolidayForUsers($Clean['HolidayForUsers']);
        $AddAcademicCalendar->SetHolidayForClasses($Clean['HolidayForClasses']);
        $AddAcademicCalendar->SetNotificationForUsers($Clean['NotificationForUsers']);
        $AddAcademicCalendar->SetNotificationForClasses($Clean['NotificationForClasses']);
        ...
        $AddAcademicCalendar->SetAllEventsDates($Clean['AllEventsDates']);
        $AddAcademicCalendar->SetCreateUserID($LoggedUser->GetUserID());

    	if (!$AddAcademicCalendar->Save())
    	{
    		$NewRecordValidator->AttachTextError(ProcessErrors($AddAcademicCalendar->GetLastErrorCode()));
    		$HasErrors = true;
    		break;
    	}
    		
    	header('location:academic_calendar_list.php?Mode=ED');
    	exit;
	break;
}
...
?>
<title>Add Academic Calendar</title>
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Add Academic Calendar</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form class="form-horizontal" name="AddAcademicCalendar" action="add_academic_calendar.php" method="post">
