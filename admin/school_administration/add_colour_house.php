...
switch ($Clean['Process'])
{
	case 1:						
		...
		$NewColourHouse = new ColourHouse();
				
		$NewColourHouse->SetHouseName($Clean['HouseName']);
		$NewColourHouse->SetHouseColour($Clean['HouseColour']);
		$NewColourHouse->SetIsActive(1);
		
		$NewColourHouse->SetCreateUserID($LoggedUser->GetUserID());

		if ($NewColourHouse->RecordExists())
		{
		...
		if (!$NewColourHouse->Save())
		{
			$NewRecordValidator->AttachTextError(ProcessErrors($NewColourHouse->GetLastErrorCode()));
			$HasErrors = true;

			break;
		}
		
		header('location:colour_houses_list.php?Mode=AS');
		exit;
	break;
}
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Add House</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form class="form-horizontal" name="AddColourHouse" action="add_colour_house.php" method="post">
