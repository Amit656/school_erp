...
elseif (isset($_POST['hdnClassClassTeacherID']))
{
    $Clean['ClassClassTeacherID'] = (int) $_POST['hdnClassClassTeacherID'];
}
...
try
{
    $ClassClassteacherToEdit = new ClassClassteacher($Clean['ClassClassTeacherID']);
}
...
switch ($Clean['Process'])
{
	case 3:
		...
		$ClassClassteacherToEdit->SetClassSectionID($Clean['ClassSectionID']);
		$ClassClassteacherToEdit->SetBranchStaffID($Clean['BranchStaffID']);

		$ClassClassteacherToEdit->SetCreateUserID($LoggedUser->GetUserID());

		if ($ClassClassteacherToEdit->RecordExists())
		{
		...
		if (!$ClassClassteacherToEdit->Save())
		{
			$NewRecordValidator->AttachTextError(ProcessErrors($ClassClassteacherToEdit->GetLastErrorCode()));
			$HasErrors = true;
			break;
		}

		header('location:class_classteachers_list.php?Mode=UD');
		exit;
		break;

	case 2:
		$Clean['ClassSectionID'] = $ClassClassteacherToEdit->GetClassSectionID();
		$Clean['BranchStaffID'] = $ClassClassteacherToEdit->GetBranchStaffID();
		$Clean['ClassID'] = $ClassClassteacherToEdit->GetClassID();
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Edit Class Teacher</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form class="form-horizontal" name="EditClassTeacher" action="edit_class_classteacher.php" method="post">
...
?>
                        ...
                        <div class="form-group">
						    <div class="col-sm-offset-2 col-lg-10">
						        ...
						        <button type="submit" class="btn btn-primary">View Applicable Classes</button>
						    </div>
						</div>
                    </div>
                </div>
...
<?php
		if ($Clean['Process'] == 2 || $Clean['Process'] == 7)
		{	
?>
			<form class="form-horizontal" name="EditClassTeacher" action="edit_class_classteacher.php" method="post">
            	<div class="panel panel-default">
...
?>
	                    ...
	                    <div class="form-group">
						    <div class="col-sm-offset-2 col-lg-10">
						    	...
						    	<input type="hidden" name="hdnClassClassTeacherID" value="<?php echo $Clean['ClassClassTeacherID']; ?>"/>
