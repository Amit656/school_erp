...
elseif (isset($_POST['hdnSectionMasterID']))
{
    $Clean['SectionMasterID'] = (int) $_POST['hdnSectionMasterID'];
}
...
try
{
   $SectionMasterToEdit = new SectionMaster($Clean['SectionMasterID']);
}
...
switch ($Clean['Process'])
{
	case 3:						
		...
		$SectionMasterToEdit->SetSectionName($Clean['SectionName']);
        $SectionMasterToEdit->SetIsActive($Clean['IsActive']);

        if ($SectionMasterToEdit->RecordExists())
        {
		...
		if (!$SectionMasterToEdit->Save())
		{
			$NewRecordValidator->AttachTextError(ProcessErrors($SectionMasterToEdit->GetLastErrorCode()));
			$HasErrors = true;
			break;
		}
		
		header('location:section_master_list.php?Mode=UD');
	    break;
        
	case 2:
		$Clean['SectionName'] = $SectionMasterToEdit->GetSectionName();
        $Clean['IsActive'] = $SectionMasterToEdit->GetIsActive();
	    break;
}

require_once('../html_header.php');
?>
<title>Edit Section Master</title>
</head>
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Edit Section Master</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form class="form-horizontal" name="EditSectionMaster" action="edit_section_master.php" method="post">
...
?>                    
                        ...
                        <div class="col-sm-offset-2 col-lg-10">
                        	...
                        	<input type="hidden" name="hdnSectionMasterID" value="<?php echo $Clean['SectionMasterID']; ?>" />
