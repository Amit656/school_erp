...
switch ($Clean['Process'])
{
	case 1:						
		...
		$NewMenu->SetMenuName($Clean['MenuName']);
		$NewMenu->SetMenuPriority($Clean['MenuPriority']);
		$NewMenu->SetIsActive(1);
		
		$NewMenu->SetCreateUserID($LoggedUser->GetUserID());

		if ($NewMenu->RecordExists())
		{
		...
		if (!$NewMenu->Save())
		{
			$NewRecordValidator->AttachTextError(ProcessErrors($NewMenu->GetLastErrorCode()));
			$HasErrors = true;

			break;
		}
		
		header('location:menus_list.php?POMode=RA');
		exit;
	break;
}
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
             ...
             <form class="form-horizontal" name="AddMenu" action="add_menu.php" method="post">
