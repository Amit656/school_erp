...
if ($LoggedUser->HasPermissionForTask(TASK_REPORT_DRIVERS) !== true)
{
...
{
    header('location:driver_report.php?Process=7');
    exit;
}
...
{
	case 5:
		if ($LoggedUser->HasPermissionForTask(TASK_DELETE_DRIVERS) !== true)
		{
			header('location:unauthorized_login_admin.php');
			exit;
		}
		...
		try
		{
			$DriverToDelete = new Driver($Clean['DriverID']);
		}
		...
		// if ($DriverToDelete->CheckDependencies())
		// {
		...
		if (!$DriverToDelete->Remove())
		{
			$RecordValidator->AttachTextError(ProcessErrors($DriverToDelete->GetLastErrorCode()));
			$HasErrors = true;
			break;
		}
    ...
    case 7:
        ...
        if ($Clean['DriverName'] != '')
        {
            $RecordValidator->ValidateStrings($Clean['DriverName'], 'Driver name should be between 1 and 60 characters.', 1, 60);
        }

        if ($Clean['ContactNumber'] != '')
        {
            $RecordValidator->ValidateStrings($Clean['ContactNumber'], 'Contact number should be between 1 and 30 characters.', 1, 15);
        }
        ...
        //get records count
        Driver::SearchDrivers($TotalRecords, true, $Filters);
        ...
        if ($TotalRecords > 0)
        {
            ...
            $AllDrivers = Driver::SearchDrivers($TotalRecords, false, $Filters, $Start, $Limit);
        }
    break;
}
...
?>                    
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Driver Report</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
            <form class="form-horizontal" name="AddDriver" action="driver_report.php" method="get">
...
?>                
                        <div class="form-group">
                            <label for="DriverName" class="col-lg-2 control-label"> Driver Name</label>
                             <div class="col-lg-3">
                        ...
                        <div class="form-group">
                                ...
                                <div class="col-lg-6">
                                    <label class="radio-inline">
                                        <input type="radio" <?php echo (($Clean['ActiveStatus'] == 0) ? 'checked="checked"' : ''); ?> name="optActiveStatus" id="ActiveStatus" value="0" checked>All Driver
                                    </label>
                                    <label class="radio-inline">
                                        <input type="radio" <?php echo (($Clean['ActiveStatus'] == 1) ? 'checked="checked"' : ''); ?> name="optActiveStatus" id="ActiveStatus" value="1">Active Driver
                                    </label>
                                    <label class="radio-inline">
                                        <input type="radio" <?php echo (($Clean['ActiveStatus'] == 2) ? 'checked="checked"' : ''); ?> name="optActiveStatus" id="ActiveStatus" value="2">In-Active Driver
                                    </label>
                                </div>
                        </div>   
                        <div class="form-group"> 
...
<?php
        if ($Clean['Process'] == 7 && $HasErrors == false)
        {
            ...
            if ($Clean['ContactNumber'] != '')
            {
                $ReportHeaderText .= ' Contact Number : ' . $Clean['ContactNumber'] . ',';
            }
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div>
                                <div class="row">
                                    <div class="col-lg-3">
                                    <div class="add-new-btn-container"><a href="add_driver.php" class="btn btn-primary<?php //echo $LoggedUser->HasPermissionForTask(TASK_ADD_MENU) === true ? '' : ' disabled'; ?>" role="button">Add New Driver</a></div>
...
<?php
                                    if ($TotalPages > 1)
                                    {
                                        ...
                                        $AllParameters = array('Process' => '7', 'DriverName' => $Clean['DriverName'], 'ContactNumber' => $Clean['ContactNumber'],);

                                        echo UIHelpers::GetPager('driver_report.php', $TotalPages, $Clean['CurrentPage'], $AllParameters);
                                    }
?>  
                                    </div>
                                    <div class="col-lg-3">
                                ...
                                <div class="row" id="RecordTableHeading">
                                    <div class="col-lg-12">
                                    	<div class="report-heading-container"><strong>Driver Report on <?php echo date('d-m-Y h:i A') . $ReportHeaderText; ?></strong></div>
                                    </div>
								</div>
                                        ...
                                        <table width="100%" class="table table-striped table-bordered table-hover" id="DataTableRecords">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Adhar Number</th>
                                                    <th>Pan Number</th>
                                                    <th>Votor IdNumber</th>
                                                    <th>DL Number</th>
                                                    <th>DL ValidityFrom</th>
                                                    <th>DL ValidityTo</th>
                                                    <th>Create User</th>
                                                    <th>IsActive</th>
                                                    <th>Create Date</th>
                                            ...
                                            <tbody>
<?php
                                    if (is_array($AllDrivers) && count($AllDrivers) > 0)
                                    {
                                        $Counter = $Start;
                                        foreach ($AllDrivers as $DriverID => $DriverDetails)
                                        {
?>
                                                <tr>
                                                    <td><?php echo ++$Counter; ?></td>
                                                    <td><?php echo $DriverDetails['DriverFirstName'] . ' ' . $DriverDetails['DriverLastName']; ?></td>
                                                    <td><?php echo $DriverDetails['FatherName']; ?></td>
                                                    <td><?php echo $DriverDetails['DOB']; ?></td>
                                                    <td><?php echo $DriverDetails['Address']; ?></td>
                                                    <td><?php echo $DriverDetails['ContactNumber']; ?></td>
                                                    <td><?php echo $DriverDetails['Email']; ?></td>
                                                    <td><?php echo (($DriverDetails['AdharNumber'] == '0') ? 'N/A' : $DriverDetails['AdharNumber']);?></td>
                                                    <td><?php echo (($DriverDetails['PanNumber'] == '') ? 'N/A' : $DriverDetails['PanNumber']); ?></td>
                                                    <td><?php echo (($DriverDetails['VoterIDNumber'] == '') ? 'N/A' : $DriverDetails['VoterIDNumber']); ?></td>
                                                    <td><?php echo $DriverDetails['DrivingLicenceNumber']; ?></td>
                                                    <td><?php echo $DriverDetails['DrivingLicenceValidityFrom']; ?></td>
                                                    <td><?php echo $DriverDetails['DrivingLicenceValidityTo']; ?></td>
                                                    <td><?php echo $DriverDetails['CreateUserName']; ?></td>
                                                   <td><?php echo (($DriverDetails['IsActive']) ? 'Yes' : 'No'); ?></td>
                                                    <td><?php echo date('d/m/Y', strtotime($DriverDetails['CreateDate'])); ?></td>
                                                    <td class="print-hidden">
<?php
                                                    // echo '<a href="edit_driver.php?Process=2&amp;DriverID=' . $DriverID . '">Edit</a>';
                                                    // echo '&nbsp;|&nbsp;';
                                                    // echo '<a class="delete-record" href="driver_report.php?Process=5&amp;DriverID=' . $DriverID . '">Delete</a>';
