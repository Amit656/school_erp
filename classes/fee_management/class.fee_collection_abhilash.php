...
class FeeCollection
{
	...
	static function SearchFeeTransactions(&$TotalRecords = 0, $GetTotalsOnly = false, $Filters = array(), $Start = 0, $Limit = 100)
	{
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT aft.feeTransactionID,aft.description, aft.createDate, SUM(afpmd.amount) AS totalTransactionAmount, afc.feeDate, apd.fatherFirstName, apd.fatherLastName, u.userName AS createUserName
												FROM afm_fee_transactions aft
												INNER JOIN afm_fee_collection afc ON afc.feeTransactionID = aft.feeTransactionID
												INNER JOIN afm_fee_payment_mode_details afpmd ON afpmd.feeTransactionID = aft.feeTransactionID
												...
												ORDER BY aft.feeTransactionID DESC LIMIT ' . (int) $Start . ', ' . (int) $Limit . ';');
			$RSSearch->Execute();
	...
	static function MonthlyFeeDueDetails(&$TotalRecords = 0, $GetTotalsOnly = false, $Filters = array(), &$OverAllSummary = array(), $Start = 0, $Limit = 100)
	{
		...
		try
		{
			...
			$RSSearchSummary = $DBConnObject->Prepare('SELECT fh.feeHeadID, fh.feeHead,
																...
																SUM(afd1.concessionAmount) AS totalConcessionAmount,
																SUM(afd1.waveOffAmount) AS totalWaveOffAmount
																FROM afm_student_fee_structure sfs
			...
			if ($RSSearchSummary->Result->num_rows > 0)
			{
				while($SearchSummaryRow = $RSSearchSummary->FetchRow())
				{
					...
					$OverAllSummary[$SearchSummaryRow->feeHeadID]['TotalConcessionAmount'] = $SearchSummaryRow->totalConcessionAmount;
					$OverAllSummary[$SearchSummaryRow->feeHeadID]['TotalWaveOffAmount'] = $SearchSummaryRow->totalWaveOffAmount;

					$OverAllSummary[$SearchSummaryRow->feeHeadID]['TotalPaidAmount'] = $SearchSummaryRow->totalAmountPaid;
					$OverAllSummary[$SearchSummaryRow->feeHeadID]['TotalDueAmount'] = $SearchSummaryRow->totalAmountPayable - $SearchSummaryRow->totalAmountPaid - $DiscountValue - $SearchSummaryRow->totalConcessionAmount - $SearchSummaryRow->totalWaveOffAmount;
				}
			}

			$RSSearchStudentFeeDetails = $DBConnObject->Prepare('SELECT asd.studentID, asd.firstName, asd.lastName, ac.classID, ac.className, asm.sectionName, CONCAT( YEAR(ay.startDate), \'-\', DATE_FORMAT(ay.endDate, \'%y\')) AS academicYearName,
																SUM(sfs.amountPayable) AS totalAmountPayable, 
																SUM(fcd.amountPaid) As totalAmountPaid,
																SUM(CASE WHEN afd.discountType = "Absolute" THEN afd.discountValue ELSE (sfs.amountPayable * afd.discountValue) / 100 END) AS firstDiscountValue, 
																SUM(CASE WHEN afd1.discountType = "Absolute" THEN afd1.discountValue ELSE (sfs.amountPayable * afd1.discountValue) / 100 END) AS secondDiscountValue,
	...
	static function GetFeeDetailsByStudent($StudentID, $MonthList = array())
	{
		$FeeDetails = array();
		try
		{
	...
	static function SearchFeeCollectionDetails(&$TotalRecords = 0, $GetTotalsOnly = false, $Filters = array(), $Start = 0, $Limit = 100)
	{
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT afc.feeTransactionID, afc.feeCollectionID, afc.feeDate, afc.totalAmount, afc.totalDiscount, afc.amountPaid, afc.createDate, afpmd.paymentMode, CONCAT( YEAR(ay.startDate), \'-\', DATE_FORMAT(ay.endDate, \'%y\')) AS academicYearName, u.userName AS createUserName, 
												asd.firstName, asd.lastName, ac.className, asm.sectionName, afcoc.amount
												FROM afm_fee_collection afc
	...
	static function GetFeeTransactionDetails($FeeCollectionID)
	{
		...
		try
		{
			...
			$RSFeeTransactionDetails = $DBConnObject->Prepare('SELECT afcd.*, afh.feeHead, afh.feeHeadID, asfs.amountPayable, aaym.monthName, afd.discountType AS firstDiscountType, afd.discountValue AS firstDiscountValue, afd.concessionAmount AS firstConcessionAmount, afd.waveOffAmount AS firstWaveOffAmount, afd1.discountType AS secondDiscountType, afd1.discountValue AS secondDiscountValue, afd1.concessionAmount AS secondConcessionAmount, afd1.waveOffAmount AS secondWaveOffAmount  
			...
			while($SearchRow = $RSFeeTransactionDetails->FetchRow())
			{
				...
				{
					$SecondDiscountAmount = $SearchRow->secondDiscountValue + $SearchRow->secondConcessionAmount + $SearchRow->firstWaveOffAmount;
				}
    			...
    			if ($RSPreviousSubmitedFeeOfMonth->Result->num_rows > 0)
    			{
    			    ...
    			    $FeeCollectionDetails[$SearchRow->monthName][$SearchRow->feeHeadID]['FeeAmount'] = $FeeAmount - $TotalSubmittedFee;
    			}
    			
				$FeeCollectionDetails[$SearchRow->monthName][$SearchRow->feeHeadID]['PaidAmount'] = $SearchRow->amountPaid;
				$FeeCollectionDetails[$SearchRow->monthName][$SearchRow->feeHeadID]['RestAmount'] = $FeeAmount - $FeeHeadDiscountAmount - $SearchRow->amountPaid -$TotalSubmittedFee;
			}
		...
		try
		{
			...
			if ($GetTotalsOnly)
			{
				$RSTotal = $DBConnObject->Prepare('SELECT asd.firstName, asd.lastName, ac.className, asm.sectionName, sfs.studentID, 
    												...
    												WHERE ass.status = \'Active\' AND fsd.academicYearMonthID IN (SELECT academicYearMonthID FROM asa_academic_year_months WHERE feePriority <= :|1)
    												'. $QueryString .'
	...
	static function GetFeeDefaulterDues($StudentID, $FeePriority, &$PreviousYearDue = 0)
	{
	    $PreviousYearDue = 0;
