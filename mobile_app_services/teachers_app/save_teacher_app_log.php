...
{
	$Clean['AppName'] = strip_tags(trim((string) $_REQUEST['AppName']));
}

if (isset($_REQUEST['Duration']))
{
	$Clean['Duration'] = strip_tags(trim((string) $_REQUEST['Duration']));
}
...
try
{
    ...
    if ($LoggedInBranchStaff->IsTeacherClockIn())
    {
        if (!APIHelpers::SaveAppLog($LoggedInBranchStaff->GetBranchStaffID(), $Clean['AppName'], $Clean['Duration'])) 
    	{
    		$Response->SetError(1);
    		$Response->SetErrorCode(UNKNOWN_ERROR);
    		$Response->SetMessage(ProcessAppErrors(UNKNOWN_ERROR));	
